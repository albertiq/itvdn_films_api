...
class TestFilms:
    ...
    def test_create_film_with_mock_db(self):
        with patch('src.db.session.add', autospec=True) as mock_sess_add, \
                patch('src.db.session.commit', autospec=True) as mock_sess_commit:
            client = app.test_client()
            data = {
                'title': 'Test title',
                'distributed_by': 'Test company',
            ...
            resp = client.post('/films', data=json.dumps(data), content_type='application/json')
            mock_sess_add.assert_called_once()
            mock_sess_commit.assert_called_once()
    ...
    def test_update_film_with_db(self):
        ...
        data = {
            'title': 'Updated title',
            'distributed_by': 'Updated company',
            'release_date': '2010-01-01',
        }
    ...
    def test_update_film_with_mock_db(self):
        with patch('src.services.film_service.FilmService.fetch_film_by_uuid') as mock_query, \
                patch('src.db.session.add', autospec=True) as mock_sess_add, \
                patch('src.db.session.commit', autospec=True) as mock_sess_commit:
            mock_query.return_value = FakeFilm()
            client = app.test_client()
            url = f'/films/1'
            data = {
                'title': 'Updated title',
